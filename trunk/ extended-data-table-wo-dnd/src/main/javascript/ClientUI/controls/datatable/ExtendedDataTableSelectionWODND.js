...
ExtendedDataTableWODND.Selection = Class.create({
	initialize: function() {
		this.ranges = [];
	},
				...
				if(this.ranges[i] && id==(this.ranges[i].indexes[0]-1)) {
					...
					if(id==(this.ranges[i].indexes[1]+1)){
						...
						if(id<this.ranges[i].indexes[1]){
							this.addRange(i, new ExtendedDataTableWODND.Range(id, id));
						} else {
							this.addRange(i + 1, new ExtendedDataTableWODND.Range(id, id));
						}
					}
				}	
			} else {
	...
	removeId: function(id) {
		...
		if(this.ranges[i]) {
			if(id==(this.ranges[i].indexes[1]) ) {
				...
				this.addRange(i+1, new ExtendedDataTableWODND.Range(id+1, this.ranges[i].indexes[1]));
	...
	initRanges: function(rangeStrRArray) {
		...
		for(var i = 0; i < l; i++) {
			...
			this.ranges[i] = new ExtendedDataTableWODND.Range(parseInt(indexStrRArray[0]), parseInt(indexStrRArray[1]));
		}
...
/* Class taken from ScrollableDataTable - unmodified */
ExtendedDataTableWODND.Range = Class.create({
...
ExtendedDataTableWODND.SelectionManager = Class.create({
	initialize: function(options, owner) {
		...
		this.selection = new ExtendedDataTableWODND.Selection();
